...
service_audit() {
    ...
    run_with_timeout 15 "
    {
        ...
        echo 'Checking critical services (ssh, iptables):'
        
        # Check if ssh is installed and running
        if systemctl is-active --quiet ssh; then
            echo 'SSH service is running.'
        else
            echo 'SSH service is not running. Please check the OpenSSH server.'
        fi
...
security_updates() {
    ...
    run_with_timeout 15 "
    {
        ...
        # Run apt-get upgrade in a non-interactive way and log output
        if ! apt-get upgrade -s | grep -i security; then
            log 'Error: Failed to list security updates.'
            return 1
        fi

        echo 'Configuring unattended-upgrades...'
        if ! apt-get install unattended-upgrades -y; then
    ...
    }" > reports/security_updates.txt 2>&1
...
custom_security_checks() {
    ...
    run_with_timeout 15 'run_custom_checks > reports/custom_checks.txt'
...
export -f disable_unnecessary_services
export -f secure_ssh
export -f secure_grub
export -f disable_ipv6
export -f configure_firewall
export -f enable_unattended_upgrades
export -f remove_unused_packages
export -f run_custom_checks

# Export the log function to make it available in subshells
log() {
    echo "[$(date +'%Y-%m-%d %H:%M:%S')] $1" >> reports/summary_report.txt
}
export -f log
...
user_group_audit() {
    run_with_timeout 15 "
    {
        ...
        awk -F: '\$3 == 0 {print \$1}' /etc/passwd >> reports/user_group_audit.txt
        echo 'Checking for users without passwords:' >> reports/user_group_audit.txt
        awk -F: '\$2 == \"\" {print \$1}' /etc/shadow >> reports/user_group_audit.txt
        echo 'Checking for weak passwords:' >> reports/user_group_audit.txt
        while IFS=: read -r user pass _ uid _ _ _ shell; do
            if [[ \$shell != */nologin && \$shell != */false ]]; then
                if [[ -z \$pass ]]; then
                    echo 'User \$user has no password set.' >> reports/user_group_audit.txt
                else
                    strength=\$(echo \"\$pass\" | cracklib-check)
                    if [[ \$strength != *\": OK\"* ]]; then
                        echo 'User \$user has a weak password: \$strength' >> reports/user_group_audit.txt
                    fi
                fi
            fi
        done < /etc/shadow
...
# Similar approach can be taken for the other audit functions
...
file_dir_audit() {
    run_with_timeout 15 "
    {
        ...
        find / -type f -perm -o+w -exec ls -l {} \; 2>/dev/null >> reports/file_permissions.txt
        echo '.ssh directories with insecure permissions:' >> reports/file_permissions.txt
        find /home -type d -name '.ssh' -exec ls -ld {} \; >> reports/file_permissions.txt
        echo 'Files with SUID/SGID bits set:' >> reports/file_permissions.txt
        find / -type f -perm /6000 -exec ls -l {} \; 2>/dev/null >> reports/file_permissions.txt
        log 'File and Directory Permissions Audit completed.'
    }"
}

# Continue with the other audits similarly...
